 ...
 */
int picoquic_parse_client_multipath_config(char* mp_config, int* src_if,
                                           struct sockaddr_storage* alt_ip,
                                           int* nb_alt_paths)
{
    int ret = 0;
    ...
    while ((token = picoquic_strsep(&str, ","))) {
        ...
        if ((valid_ip == 1) && (valid_index == 1)) {
            (*nb_alt_paths)++;
            /* If more than PICOQUIC_NB_PATH_TARGET alt paths are specified, the
            ...
            case picoquic_packet_loop_after_receive:
                ...
                if (!cb_ctx->address_updated && cb_ctx->cnx_client->path[0]->local_addr.ss_family != 0) {
                    uint16_t updated_port = (cb_ctx->cnx_client->path[0]->local_addr.ss_family == AF_INET)
                                            ? ((struct sockaddr_in*)&cb_ctx->cnx_client->path[0]
                                    ->local_addr)
                                                    ->sin_port
                                            : ((struct sockaddr_in6*)&cb_ctx->cnx_client->path[0]
                                    ->local_addr)
                                                    ->sin6_port;
                    if (updated_port != 0) {
                        cb_ctx->address_updated = 1;
                        picoquic_store_addr(
                                &cb_ctx->client_address,
                ...
                if (picoquic_get_cnx_state(cb_ctx->cnx_client) == picoquic_state_client_almost_ready && cb_ctx->notified_ready == 0) {
                    ...
                    if (cb_ctx->cnx_client->alpn != NULL) {
                        ...
                        picoquic_log_app_message(cb_ctx->cnx_client, "Negotiated ALPN: %s",
                                                 cb_ctx->cnx_client->alpn);
                    ...
                    if (picoquic_get_cnx_state(cb_ctx->cnx_client) == picoquic_state_ready && cb_ctx->multipath_probe_done == 0) {
                        for (int i = 0; i < cb_ctx->nb_alt_paths; i++) {
                            if ((ret = picoquic_probe_new_path_ex(
                                    cb_ctx->cnx_client,
                                    (struct sockaddr*)&cb_ctx->server_address,
                                    (struct sockaddr*)&cb_ctx->client_alt_address[i],
                                    cb_ctx->client_alt_if[i], picoquic_get_quic_time(quic),
                                    0))
                                != 0) {
                                picoquic_log_app_message(
                                        cb_ctx->cnx_client,
                                        "Probe new path failed with exit code %d\n", ret);
                            } else {
                                picoquic_log_app_message(
                                        cb_ctx->cnx_client,
                                        "New path added, total path available %d\n",
                    ...
                    if (cb_ctx->cnx_client->remote_parameters.prefered_address.is_defined && !cb_ctx->migration_to_preferred_finished) {
                        if (picoquic_compare_addr(
                                (struct sockaddr*)&cb_ctx->server_address,
                                (struct sockaddr*)&cb_ctx->cnx_client->path[0]->peer_addr)
                            != 0) {
                            fprintf(stdout, "Migrated to server preferred address!\n");
                            picoquic_log_app_message(cb_ctx->cnx_client, "%s",
                                                     "Migrated to server preferred address!");
                            ...
                            fprintf(stdout, "Attempting migration to server "
                                            "preferred address.\n");
                            picoquic_log_app_message(
                                    cb_ctx->cnx_client, "%s",
                                    "Attempting migration to server preferred address.");
                        } else if (cb_ctx->cnx_client->nb_paths == 1 && cb_ctx->migration_to_preferred_started) {
                            fprintf(stdout, "Could not migrate to server preferred address!\n");
                            picoquic_log_app_message(
                                    cb_ctx->cnx_client, "%s",
                                    "Could not migrate to server preferred address!");
                            cb_ctx->migration_to_preferred_finished = 1;
                        }
                    }

                    /* Execute the migration trials
                     * The actual change of sockets is delegated to the packet loop
                     * function, so it can be integrated with other aspects of socket
                     * management. If a new socket is needed, two special error codes
                     * will be used.
                     */
                    if (cb_ctx->force_migration && cb_ctx->migration_started == 0 && cb_ctx->address_updated && picoquic_get_cnx_state(cb_ctx->cnx_client) == picoquic_state_ready && (cb_ctx->cnx_client->cnxid_stash_first != NULL || cb_ctx->force_migration == 1) && (!cb_ctx->cnx_client->remote_parameters.prefered_address.is_defined || cb_ctx->migration_to_preferred_finished)) {
                        ...
                        if (cb_ctx->cnx_client->path[0]->p_local_cnxid != NULL) {
                            ...
                            /* Special case of forced migration after preferred
                             * address migration */
                            memset(&cb_ctx->client_cid_before_migration, 0,
                                   sizeof(picoquic_connection_id_t));
                        }
                        switch (cb_ctx->force_migration) {
                            case 1:
                                fprintf(stdout, "Switch to new port. Will test NAT "
                                                "rebinding support.\n");
                            ...
                            case 2:
                                ...
                                if (mig_ret != 0) {
                                    if (mig_ret == PICOQUIC_ERROR_MIGRATION_DISABLED) {
                                        fprintf(stdout, "Migration disabled, cannot "
                                                        "test CNXID renewal.\n");
                                    } else {
            ...
            case picoquic_packet_loop_after_send:
                if (picoquic_get_cnx_state(cb_ctx->cnx_client) == picoquic_state_disconnected) {
                    ...
                    printf("Connection established. Version = %x, I-CID: %llx, "
                           "verified: %d\n",
                           picoquic_supported_versions[cb_ctx->cnx_client->version_index]
                                   .version,
                           (unsigned long long)picoquic_val64_connection_id(
                                   picoquic_get_logging_cnxid(cb_ctx->cnx_client)),
                    ...
                    if (!cb_ctx->zero_rtt_available && !cb_ctx->is_siduck && !cb_ctx->is_quicperf) {
                        ...
                        ret = picoquic_demo_client_start_streams(
                                cb_ctx->cnx_client, cb_ctx->demo_callback_ctx,
                ...
                picoquic_quic_config_t* config, char const* client_scenario_text)
{
    ...
    ret = picoquic_get_server_address(ip_address_text, server_port,
                                      &loop_cb.server_address, &is_name);
    if (sni == NULL) {
        sni = ip_address_text;
    }
    ...
    if (ret == 0) {
        if (config->ticket_file_name == NULL) {
            ret = picoquic_config_set_option(config, picoquic_option_Ticket_File_Name,
                                             ticket_store_filename);
        }
        if (ret == 0 && config->token_file_name == NULL) {
            ret = picoquic_config_set_option(config, picoquic_option_Token_File_Name,
    ...
    if (config->alpn == NULL || config->proposed_version == 0) {
        ...
        if (picoquic_demo_client_get_alpn_and_version_from_tickets(
                qclient, sni, config->alpn, config->proposed_version, current_time,
                &ticket_alpn, &ticket_version)
            == 0) {
            if (ticket_alpn != NULL) {
    ...
    if (ret == 0) {
        if (config->alpn != NULL && (strcmp(config->alpn, "siduck") == 0 || strcmp(config->alpn, "siduck-00") == 0)) {
            ...
            ret = demo_client_parse_scenario_desc(client_scenario_text, &client_sc_nb,
                                                  &client_sc);
            if (ret != 0) {
                fprintf(stdout, "Cannot parse the specified scenario.\n");
                return -1;
            } else {
                ret = picoquic_demo_client_initialize_context(
                        &callback_ctx, client_sc, client_sc_nb, config->alpn,
                        config->no_disk, 0);
    ...
    if (ret == 0) {
        ...
        if (cnx_client == NULL) {
            ...
            /* Set PMTUD policy to delayed on the client, leave to default=basic
             * on server */
            ...
            if (is_siduck) {
                ...
                picoquic_set_callback(cnx_client, picoquic_demo_client_callback,
                                      &callback_ctx);
            ...
            if (ret == 0) {
                ...
                printf("Starting client connection. Version = %x, I-CID: %llx\n",
                       ...
                       (unsigned long long)picoquic_val64_connection_id(
                               picoquic_get_logging_cnxid(cnx_client)));
            ...
            if (ret == 0 && !is_siduck && !is_quicperf) {
                if (picoquic_is_0rtt_available(cnx_client) && (config->proposed_version & 0x0a0a0a0a) != 0x0a0a0a0a) {
                    ...
                    fprintf(
                            stdout, "Max stream id bidir remote after 0rtt = %d (%d)\n",
                            (int)cnx_client->max_stream_id_bidir_remote,
                            (int)cnx_client->remote_parameters.initial_max_stream_id_bidir);
    ...
    if (ret == 0) {
        if (config->multipath_alt_config != NULL) {
            picoquic_parse_client_multipath_config(
                    config->multipath_alt_config, loop_cb.client_alt_if,
                    loop_cb.client_alt_address, &loop_cb.nb_alt_paths);
        }
        ...
        ret = picoquic_packet_loop(qclient, 0, loop_cb.server_address.ss_family, 0,
                                   config->socket_buffer_size, config->do_not_use_gso,
                                   client_loop_cb, &loop_cb);
    }
    ...
    if (ret == 0) {
        ...
        if (cnx_client->ack_ctx[picoquic_packet_context_application]
                    .ecn_ect0_total_local
            != 0
            || cnx_client->ack_ctx[picoquic_packet_context_application]
                       .ecn_ect1_total_local
               != 0
            || cnx_client->ack_ctx[picoquic_packet_context_application]
                       .ecn_ce_total_local
               != 0) {
            fprintf(stdout,
                    "ECN was received (ect0: %" PRIu64 ", ect1: %" PRIu64
            ", ce: %" PRIu64 ").\n",
                    cnx_client->ack_ctx[picoquic_packet_context_application]
                            .ecn_ect0_total_local,
                    cnx_client->ack_ctx[picoquic_packet_context_application]
        ...
        if (cnx_client->pkt_ctx[picoquic_packet_context_application]
                    .ecn_ect0_total_remote
            != 0
            || cnx_client->pkt_ctx[picoquic_packet_context_application]
            ...
            || cnx_client->pkt_ctx[picoquic_packet_context_application]
               ...
               != 0) {
            fprintf(stdout,
                    "ECN was acknowledged (ect0: %" PRIu64 ", ect1: %" PRIu64
            ", ce: %" PRIu64 ").\n",
                    cnx_client->pkt_ctx[picoquic_packet_context_application]
                            .ecn_ect0_total_remote,
                    cnx_client->pkt_ctx[picoquic_packet_context_application]
        ...
        if (config->desired_version != 0) {
            ...
            if (v == config->desired_version) {
                ...
                fprintf(stdout,
                        "Could not negotiate version 0x%" PRIx32
                ", used version 0x%" PRIu32 ".\n",
        ...
        if (picoquic_get_data_received(cnx_client) > 0) {
            ...
            if (duration_usec > 0) {
                if (is_quicperf) {
                    ...
                    picoquic_log_app_message(
                            cnx_client, "Received %" PRIu64 " bytes in %f seconds, %f Mbps.",
                            picoquic_get_data_received(cnx_client), duration_usec,
                            ((double)quicperf_ctx->data_received) * 8.0 / duration_usec);
                } else {
                    double receive_rate_mbps = 8.0 * ((double)picoquic_get_data_received(cnx_client)) / duration_usec;
                    fprintf(stdout,
                            "Received %" PRIu64 " bytes in %f seconds, %f Mbps.\n",
                            picoquic_get_data_received(cnx_client),
                            duration_usec / 1000000.0, receive_rate_mbps);
                    picoquic_log_app_message(
                            cnx_client, "Received %" PRIu64 " bytes in %f seconds, %f Mbps.",
                            picoquic_get_data_received(cnx_client), duration_usec / 1000000.0,
                            receive_rate_mbps);
                }
                /* Print those for debugging the effects of ack frequency and
                 * flow control */
                ...
                printf("ack_delay_remote: %" PRIu64 " ... %" PRIu64 "\n",
                        cnx_client->min_ack_delay_remote,
                        cnx_client->max_ack_delay_remote);
                printf("max_ack_gap_remote: %" PRIu64 "\n",
                        cnx_client->max_ack_gap_remote);
                printf("ack_delay_local: %" PRIu64 " ... %" PRIu64 "\n",
                        cnx_client->min_ack_delay_local,
                        cnx_client->max_ack_delay_local);
    ...
    if (qclient != NULL) {
        ...
        if (sni != NULL && loop_cb.saved_alpn != NULL && 0 == picoquic_get_ticket(qclient->p_first_ticket, current_time, sni, (uint16_t)strlen(sni), loop_cb.saved_alpn, (uint16_t)strlen(loop_cb.saved_alpn), 0, &ticket, &ticket_length, NULL, 0)) {
            fprintf(stdout, "Received ticket from %s (%s):\n", sni,
                    loop_cb.saved_alpn);
            picoquic_log_picotls_ticket(stdout, picoquic_null_connection_id, ticket,
                                        ticket_length);
        }
